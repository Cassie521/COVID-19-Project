...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[CO, CE]{Xinyi Yao, 1005217766}
...
pivot_longer(-c(reported_date), 
             names_to = "case_situation", 
             values_to = "number_of_persons") %>%
mutate(case_situation = str_to_sentence(case_situation)) %>%
mutate(case_situation=fct_relevel(case_situation,"Deceased",after=2))
...
```{r cases_vis}
reported %>%
ggplot(aes(x=reported_date,y=number_of_persons,fill=case_situation))+
...
labs(title="Cases reported by day in Toronto, Canada",
      ...
      x = "Date",
      y="Case count",
      caption= str_c("Created by: Xinyi Yao for STA303/1002, U of T\n", 
...
"Source: Ontario Ministry of Health, Integrated Public Health Information System and CORES\n",
  ...
  scale_y_continuous(limits=c(0,max(outbreak$total_cases)),
                     breaks=seq(0,max(outbreak$total_cases),
                                by=2000)) +
  theme(legend.title=element_blank(),legend.position=c(0.15,0.8)) +
...
```{r}
view(nbhoods_shape_raw)
```


```{r nbhood_dw_2, warning=FALSE}
...
ggplot()+
  geom_sf(data=nbhoods_final,aes(fill=percentage,geometry=geometry)) + 
  theme_map() + 
...
ggplot(data=nbhoods_final)+
  geom_sf(aes(fill=rate_per_100000,geometry=geometry)) +
  theme_map() +
  theme(legend.position="right") +
...
ggplot(data=nbhoods_final) + 
  geom_sf(data=nbhoods_final,aes(fill=nbhood_type,geometry=geometry))+
  theme_map()+
  theme(legend.position="right")+
  labs(title="COVID-19 cases per 100,000, by neighbourhood in Toronto, Canada",
       caption=str_c("Created by: Xinyi Yao for STA303/1002, U of T\n",
"Income data sourse: Census Profile 98-316-X2016001 via OpenData Toronto\n",
"COVID data sourse: Ontario Ministry of Health, Integrated Public\n",
